...
/**
 * @brief 从路径中读取文件并计算md5值
 * @attention 默认文件一定是存在的
 * @return 十六进制字符串
*/
inline std::string md5_hash_hex_from_file(std::string const &input_path)
{
    std::ifstream file(input_path, std::ios::binary);
    if (!file.is_open())
    {
        std::cerr << "Error: Unable to open file " << input_path << std::endl;
        return "";
    }
