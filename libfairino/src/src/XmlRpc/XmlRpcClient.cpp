...
XmlRpcClient::XmlRpcClient(const char* host, int port, const char* uri/*=0*/)
{
  ...
  logger_info("XmlRpcClient new client: host %s, port %d.", host, port);
...
XmlRpcClient::close()
{
  ...
  logger_info("XmlRpcClient::close: fd %d.", getfd());
  _connectionState = NO_CONNECTION;
  _disp.exit();
...
XmlRpcClient::execute(const char* method, XmlRpcValue const& params, XmlRpcValue& result)
{
  ...
  logger_info("XmlRpcClient::execute: method %s (_connectionState %d).", method, _connectionState);
  ...
  if (_executing)
  {
    logger_error("other client is executing RPC.");
    return false;
  }
  ...
  if ( ! setupConnection())
  {
    logger_error("connect to rpc server fail.");
    return false;
  }

  if ( ! generateRequest(method, params))
  {
    logger_error("generate request fail.");
    return false;
  }
  ...
  logger_info("XmlRpcClient::execute: method %s completed.", method);
  _response = "";
  return true;
}
...
XmlRpcClient::handleEvent(unsigned eventType)
{
  if (eventType == XmlRpcDispatch::Exception)
  {
    if (_connectionState == WRITE_REQUEST && _bytesWritten == 0)
    {
      ...
      logger_error("Error in XmlRpcClient::handleEvent: could not connect to server (%s).", XmlRpcSocket::getErrorMsg().c_str());
    } 
    else
    {
      // XmlRpcUtil::error("Error in XmlRpcClient::handleEvent (state %d): %s.", 
                        // _connectionState, XmlRpcSocket::getErrorMsg().c_str());
      logger_error("Error in XmlRpcClient::handleEvent (state %d): %s.",  _connectionState, XmlRpcSocket::getErrorMsg().c_str());
    }
    return 0;
  }
...
XmlRpcClient::setupConnection()
{
  ...
  {
    logger_error("there is a rpc connection between server-client.");
    close();
  }
...
XmlRpcClient::doConnect()
{
  ...
  if (fd < 0)
  {
    // XmlRpcUtil::error("Error in XmlRpcClient::doConnect: Could not create socket (%s).%d.", XmlRpcSocket::getErrorMsg().c_str(),fd);
    logger_error("Error in XmlRpcClient::doConnect: Could not create socket (%s).%d.", XmlRpcSocket::getErrorMsg().c_str(),fd);
    return false;
  }

  // XmlRpcUtil::log(3, "XmlRpcClient::doConnect: fd %d.", fd);
  logger_info("XmlRpcClient::doConnect: fd %d.", fd);
  this->setfd(fd);
  ...
  if ( ! XmlRpcSocket::setNonBlocking(fd))
  {
    ...
    logger_error("Error in XmlRpcClient::doConnect: Could not set socket to non-blocking IO mode (%s).", XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
  ...
  if ( ! XmlRpcSocket::connect(fd, _host, _port))
  {
    ...
    logger_error("Error in XmlRpcClient::doConnect: Could not connect to server (%s).", XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
...
XmlRpcClient::generateRequest(const char* methodName, XmlRpcValue const& params)
{
  ...
  logger_info("XmlRpcClient::generateRequest: header is %d bytes, content-length is %d.", header.length(), body.length());
...
XmlRpcClient::writeRequest()
{
  if (_bytesWritten == 0)
  {
    ...
    logger_info("XmlRpcClient::writeRequest (attempt %d):\n%s", _sendAttempts+1, _request.c_str());
  }
  ...
  if ( ! XmlRpcSocket::nbWrite(this->getfd(), _request, &_bytesWritten)) {
    ...
    logger_error("Error in XmlRpcClient::writeRequest: write error (%s).",XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
    
  // XmlRpcUtil::log(3, "XmlRpcClient::writeRequest: wrote %d of %d bytes.", _bytesWritten, _request.length());
  logger_info("XmlRpcClient::writeRequest: wrote %d of %d bytes.", _bytesWritten, _request.length());
...
XmlRpcClient::readHeader()
{
  ...
  if ( ! XmlRpcSocket::nbRead(this->getfd(), _header, &_eof) ||
    ...
    if (getKeepOpen() && _header.length() == 0 && _sendAttempts++ == 0) {
      ...
      logger_error("XmlRpcClient::readHeader: re-trying connection");
      XmlRpcSource::close();
      _connectionState = NO_CONNECTION;
      _eof = false;
    ...
    logger_error("Error in XmlRpcClient::readHeader: error while reading header (%s) on fd %d.", XmlRpcSocket::getErrorMsg().c_str(), getfd());

    return false;
  }

  // XmlRpcUtil::log(4, "XmlRpcClient::readHeader: client has read %d bytes", _header.length());
  logger_info("XmlRpcClient::readHeader: client has read %d bytes", _header.length());
  ...
  if (bp == 0) {
    if (_eof)          // EOF in the middle of a response is an error
    {
      ...
      logger_error("Error in XmlRpcClient::readHeader: EOF while reading header");
  ...
  if (lp == 0) {
    ...
    logger_error("Error XmlRpcClient::readHeader: No Content-length specified");
  ...
  if (_contentLength <= 0) {
    ...
    logger_error("Error in XmlRpcClient::readHeader: Invalid Content-length specified (%d).", _contentLength);
    return false;
  }
  	
  // XmlRpcUtil::log(4, "client read content length: %d", _contentLength);
  logger_info("client read content length: %d", _contentLength);
...
XmlRpcClient::readResponse()
{
  ...
  if (int(_response.length()) < _contentLength) {
    if ( ! XmlRpcSocket::nbRead(this->getfd(), _response, &_eof)) {
      ...
      logger_error("Error in XmlRpcClient::readResponse: read error (%s).",XmlRpcSocket::getErrorMsg().c_str());
      return false;
    }
    ...
    if (int(_response.length()) < _contentLength) {
      if (_eof) {
        ...
        logger_error("Error in XmlRpcClient::readResponse: EOF while reading response");
        return false;
      }
      return true;
    }
  ...
  logger_info("XmlRpcClient::readResponse (read %d bytes)", _response.length());
...
XmlRpcClient::parseResponse(XmlRpcValue& result)
{
  ...
  if ( ! XmlRpcUtil::findTag(METHODRESPONSE_TAG,_response,&offset)) {
    ...
    logger_error("Error in XmlRpcClient::parseResponse: Invalid response - no methodResponse. Response:\n%s", _response.c_str());
    return false;
  }
      ...
      XmlRpcUtil::nextTagIs(FAULT_TAG,_response,&offset) && (_isFault = true))
  {
    if (!result.fromXml(_response, &offset))
    {
      ...
      logger_error("Error in XmlRpcClient::parseResponse: Invalid response value. Response:\n%s", _response.c_str());
      _response = "";
      return false;
    }
    /* rpc fail, you may try to rpc not existing method. */
    if (_isFault == true)
    {
      logger_error("check reponse xml, method may not exist.");
      _response = "";
      return false;
    }
  } else {
    // XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid response - no param or fault tag. Response:\n%s", _response.c_str());
    logger_error("Error in XmlRpcClient::parseResponse: Invalid response - no param or fault tag. Response:\n%s", _response.c_str());
    _response = "";
    return false;
  }
